...
public class LoginController extends Controlador {
    ...
    @Inject
    public LoginController(AtletaLogado atletaLogado, Result resultado, AtletaDAO atletaDao, LoginNegocio loginNegocio) {
        super(resultado);
    ...
    public void salvar(Atleta atleta) {
        ...
        if( this.atletaDao.salvar(atleta).equals(null) )
            this.resultado.include("error", "mensagem.atleta.cadastro.error");
        else
            this.resultado.include("success", "mensagem.atleta.cadastro.sucesso");
        this.resultado.redirectTo(this).form();
    }
